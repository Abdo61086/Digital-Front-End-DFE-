...
class Notch_Filter:
    ...
    def apply_notch_filter(self, filter_coeff, x_n):
        filter_coeff_fp = [round(sample * (1 << 14)) for sample in filter_coeff]
        # [print(hex(int(item) & 0xFFFF)) for item in filter_coeff_fp]
               
        b0, b1, b2, a1, a2 = filter_coeff_fp
    ...
    def notch_plot(self, notch_input, notch_output) :
        notch_input_float = [sample / (1 << 14) for sample in notch_input]
        notch_output_float = [sample / (1 << 14) for sample in notch_output]

        ### fft of xt for plotting
        notch_input_fft = np.fft.fft(notch_input_float)
        notch_input_fft_freq = np.fft.fftfreq(notch_input_fft.size, 1/self.Fs)


        ### fft of xf_d for plotting
        notch_output_fft = np.fft.fft(notch_output_float)
